...
def train_loop(args, config, model, train_data, Loss_Func, opt):    
    ...
    for sents in tqdm(train_iterator, desc="Training Progress"):
...
def encode_sentence_batch(sentences, model, vocab, pad_token_id):
    batch_tokens = []
    ...
    padded_tokens = torch.full((len(batch_tokens), max_len), pad_token_id, dtype=torch.long)
    for i, tokens in enumerate(batch_tokens):
        padded_tokens[i, :len(tokens)] = torch.tensor(tokens, dtype=torch.long)

    src_padding_mask = padded_tokens.ne(pad_token_id).to(device)
    encoded = model.encoder(input_ids=padded_tokens.to(device), attention_mask=src_padding_mask)
    print("User Encoding Message Complete")
    return encoded, src_padding_mask

def add_noise_to_encoded(encoded, model, snr, channel):
    noise = SNR_to_noise(snr)
    channel_enc_inp = model.channel_encoder(encoded)
    fading_signal = channel_Fading(channel_enc_inp, noise, channel)
    noisy_output = model.channel_decoder(fading_signal)
    return noisy_output

def decode_and_convert_back(noisy_input, model, src_padding_mask, vocab):
    batch_size = noisy_input.size(0)
    outputs = torch.ones(batch_size, 1, dtype=torch.long).fill_(vocab["<start>"]).to(device)
    idx_to_seq = {idx: word for word, idx in vocab.items()}

    for _ in range(Config.max_position_embeddings):
        trg_padding_mask = outputs.ne(vocab["<pad>"]).to(device)
        dec_output = model.decoder(
            input_ids=outputs, 
        ...
        logits = model.lm_head(dec_output)
        logits = logits + model.final_logits_bias.to(logits.device)
        _, next_words = torch.max(logits[:, -1, :], dim=-1)  

        next_words = next_words.unsqueeze(-1) 
        outputs = torch.cat([outputs, next_words], dim=1)

  
        if all(next_word.item() == vocab["<end>"] for next_word in next_words):
            break
    ...
    for tokens in outputs:
        decoded_sentences.append(
            " ".join(idx_to_seq[token.item()] for token in tokens if token.item() not in [vocab["<pad>"], vocab["<end>"]])
        )
    return decoded_sentences
def save_to_bin_file(tensor, filename):
    array = tensor.detach().cpu().numpy().astype(np.float32)
    byte_array = array.tobytes()
    with open(filename, 'wb') as f:
        f.write(byte_array)  
def call_ns3_program(run_ns3_dir):
    command = f"./ns3 run 'sat-cbr-example'"
    try:
        subprocess.run(command, shell=True, check=True, cwd=run_ns3_dir)
        #print("call NS3 success")
    except subprocess.CalledProcessError as e:
        print(f"call NS3 failed: {e}")

def read_bin_file_to_tensor(filename, tensor_shape, dtype=torch.float32, device='cuda'):
    try:
        with open(filename, 'rb') as f:
            byte_data = f.read()

        if len(byte_data) == 0:
            raise ValueError("文件為空，未接收到數據")

        float_data = np.frombuffer(byte_data, dtype=np.float32)

        total_elements = np.prod(tensor_shape) 
        if float_data.size < total_elements:
            print(f"數據不足，填充 {total_elements - float_data.size} 個零")
            float_data = np.pad(float_data, (0, total_elements - float_data.size), mode='constant')
        elif float_data.size > total_elements:

            print(f"數據過多，裁剪到符合形狀 {tensor_shape}")
            float_data = float_data[:total_elements]

        # 確保 device 是字符串
        if isinstance(device, str):
            device = torch.device(device)

        tensor = torch.tensor(float_data, dtype=dtype).reshape(tensor_shape).to(device)
        return tensor

    except Exception as e:
        raise RuntimeError(f"讀取或轉換檔案時發生錯誤: {e}")

def decode_from_ns3_output(ns3_output_path, tensor_shape, model, src_padding_mask, vocab):
    decoded_tensor = read_bin_file_to_tensor(ns3_output_path, tensor_shape)
    decoded_sentences = decode_and_convert_back(decoded_tensor, model, src_padding_mask, vocab)
    return decoded_sentences

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
bin_file = "encode_msg.bin"
ns3_dir = os.path.expanduser("~/bake/source/ns-3.37/contrib/satellite/examples")
run_ns3_dir = os.path.expanduser("~/bake/source/ns-3.37")
def main(Config):
    def get_args():
    ...
    test_sentences = ["Parliament approved the legislative resolution"]
    snr = 9
    channel = 'Pass'

    encoded_input, src_padding_mask = encode_sentence_batch(test_sentences, model, vocab, Config.pad_token_id)
    noisy_output = add_noise_to_encoded(encoded_input, model, snr, channel)
    #sns3 part 
    #tensor to bit stream
    save_to_bin_file(noisy_output, os.path.join(ns3_dir, bin_file))
    #call sns3
    call_ns3_program(run_ns3_dir)
    
    #load data from bin file and predict
    ns3_output_path = os.path.join(ns3_dir, 'output.bin')
    tensor_shape = noisy_output.shape
    predictions = decode_from_ns3_output(ns3_output_path, tensor_shape, model, src_padding_mask, vocab)
    print("User decoding Message Complete")


    for i, (input_sentence, output_sentence) in enumerate(zip(test_sentences, predictions)):
        print(f"Input Sentence {i+1}: {input_sentence}")
        print(f"Output Sentence {i+1}: {output_sentence}")
        print("-" * 50)
